...
$extension = Resolve-TaskPath $ExtensionPath 'apps\extension\dist' $workspace
$nativeHost = Resolve-TaskPath $NativeHostPath 'dist\apps\native-host\relay-native-host.exe' $workspace
$mcpAdapter = Resolve-TaskPath '' 'dist\packages\mcp-stdio-adapter\src\main.js' $workspace
$brokerEntry = Resolve-TaskPath '' 'dist\apps\broker\src\main.js' $workspace
...
5. The extension displays a profile-local proposed nickname. From $workspace, create its one-time code with:

   pnpm pair --nickname "<the displayed nickname>" --db "$data\relay.sqlite"

   Enter the returned pairing_code and choose **Save and connect**. The code expires after 10 minutes by default.
6. Wait for Status: connected, then confirm the broker context lists the final nickname and at least one browser window.
7. Repeat for every profile. Never reuse one pairing code across profiles.

Reset pairing only when intentionally replacing that profile endpoint. Reset does not delete browser history, tabs, or the browser profile.
"@
